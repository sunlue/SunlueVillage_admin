	...
	export default{
		...
		mounted() {
			...
			this.$store.dispatch('readVillageDataDetails', {
				...
				this.form.data.region=result.region;
				this.form.data.type=result.type+'';
				...
				this.form.data.nation=result.nation?result.nation.split(','):[];
		...
		methods: {
			...
			handleSubmit () {
				...
				this.formValidate('formVal',function(){
					that.$store.dispatch('updateVillageData',data).then((result) => {
