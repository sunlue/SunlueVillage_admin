<template>
	<Card class="create-village-scenic">
		<p slot="title">
		...
		<Form ref="formVal" :model="form.data" :rules="form.rule" :label-width="120">
			<Tabs v-model="tabs">
				<TabPane :label="$t('basis_info')" name="tab_basis">
					<Row>
						<Col span="15">
							<Row>
								<Col span="24">
									<FormItem label="两新组织名称" prop="name">
										<Input v-model="form.data.name"></Input>
									</FormItem>
								</Col>
								<Col span="24">
									<FormItem label="两新组织简介">
										<Input type="textarea" v-model="form.data.intro"></Input>
									</FormItem>
								</Col>
								<Col span="12">
									<FormItem label="经度">
								...
								<Col span="24">
									<FormItem label="两新组织特色">
										<Input type="textarea" v-model="form.data.features"></Input>
									</FormItem>
								</Col>
								<Col span="12">
									<FormItem label="游玩时长">
				...
				<TabPane label="两新组织介绍" name="tab_content">
		...
		data() {
			...
			return {
				form:{
					...
					rule:{
						name:[
							{required: true,trigger: 'blur',message: that.$t('please')+that.$t('enter')+'两新组织名称'},
						]
					}
				},
				tabs:'tab_basis'
		...
		methods: {
			...
			formValidate(name,callback){
				this.$refs[name].validate((valid) => {
					...
					if(data.thumbnail.length<1){
						this.$Message.error(this.$t('please')+this.$t('upload')+'两新组织图片');
						this.tabs='tab_basis';
						return false;
					}else if(data.content.length<1){
						this.$Message.error(this.$t('please')+this.$t('enter')+'两新组织介绍');
			...
			handleSubmit () {
				...
				this.formValidate('formVal',function(){
					that.$store.dispatch('createVillageScenic',data).then((result) => {
