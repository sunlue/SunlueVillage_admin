	...
	<Card class="create-village-scenic">
		...
		<Form ref="formVal" :model="form.data" :rules="form.rule" :label-width="100">
			<Tabs v-model="tabs">
				...
				<TabPane label="景区介绍" name="tab_content">
...
<script>
	...
	export default{
		...
		mounted() {
			this.form.data=this.updata
			this.$refs.content.setContent(this.updata.content)
		},
		methods: {
			...
			formValidate(name,callback){
				this.$refs[name].validate((valid) => {
					...
					if(data.thumbnail.length<1){
						this.$Message.error(this.$t('please')+this.$t('upload')+'景区图片');
						this.tabs='tab_basis';
						return false;
					}else if(data.content.length<1){
						this.$Message.error(this.$t('please')+this.$t('enter')+'景区介绍');
			...
			handleSubmit () {
				...
				this.formValidate('formVal',function(){
					that.$store.dispatch('updateVillageScenic',data).then((result) => {
